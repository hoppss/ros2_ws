...
#include "rcl_action_tutorials/action/fibonacci.hpp"
...
{
public:
  using Fibonacci = rcl_action_tutorials::action::Fibonacci;
    ...
    : Node("minimal_action_client", node_options), goal_done_(false)
  {
    ...
    this->timer_ =
        this->create_wall_timer(std::chrono::milliseconds(500), std::bind(&MinimalActionClient::send_goal, this));
  }
...
private:
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Next number in sequence received: %" PRId32, feedback->partial_sequence.size());
  }
